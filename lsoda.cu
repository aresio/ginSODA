/*
lsoda.cu: porting of LSODA to CUDA.
...
void SetConstants( unsigned int variables, unsigned int parameters, unsigned int time_in, unsigned int cs_lun, unsigned int threads, bool dump = false) {

	cudaMemcpyToSymbol( DEV_CONST_VARIABLES,   &variables, sizeof(variables));		// number of ODEs
	CudaCheckError();
	cudaMemcpyToSymbol( DEV_CONST_PARAMETERS, &parameters, sizeof(parameters));		// number of parameters
	CudaCheckError();
	cudaMemcpyToSymbol( DEV_CONST_TIMESLUN,  &time_in, sizeof(time_in));			// number of time instants
	CudaCheckError();
	cudaMemcpyToSymbol( DEV_CONST_SAMPLESPECIES,  &cs_lun, sizeof(cs_lun));			// number of species to sample
	CudaCheckError();
	cudaMemcpyToSymbol( DEV_ACTUAL_THREADS, &threads, sizeof(threads));				// actual threads
	CudaCheckError();
	...
	unsigned int ac_back = 0;

	cudaMemcpyFromSymbol( &s_back, DEV_CONST_VARIABLES, sizeof(s_back) );
	CudaCheckError();
	if (dump)
		printf( " * Verified on GPU: species was set to %d\n", s_back);

	cudaMemcpyFromSymbol( &r_back, DEV_CONST_PARAMETERS, sizeof(r_back) );
	CudaCheckError();
	if (dump)
	...
	cudaMemcpyFromSymbol( &cs_back, DEV_CONST_SAMPLESPECIES, sizeof(cs_back) );
	CudaCheckError();
	if (dump)
		printf( " * Verified on GPU: number of sampled species was set to %d\n", cs_back);

	cudaMemcpyFromSymbol( &ac_back, DEV_ACTUAL_THREADS, sizeof(ac_back) );
	CudaCheckError();
	if (dump)
		printf( " * Verified on GPU: number of actual threads was set to %d\n", ac_back);
...
__device__ int dlsoda_(int *neq, double *y, double *t, double *tout, int *itol, double *rtol, double *atol, 
	int *itask, int *istate, int *iopt, double *rwork, int *lrw, int *iwork, int *liw, int *jt, struct ginSODACommonBlock *common, int* debug,  double* costanti)
{
...
L110:
    ...
    lf0 = lyh + nyh;
    myFex(neq, t, y, &rwork[lf0 -1], costanti); //fixed neq y
    nfe = 1;
...
L290:
	...
	/* ----------------------------------------------------------------------- */
    dstoda_(neq, y, &rwork[lyh -1], &nyh, &rwork[lyh -1], &rwork[lewt -1], &rwork[lsavf -1], &rwork[lacor -1], &rwork[lwm -1], &iwork[liwm -1], common, costanti); 
    kgo = 1 - kflag;
    switch (kgo) {
		case 1:  goto L300;
		case 2:  goto L530;
...
L220:
    ...
    myFex(neq, &tn, y, savf, costanti); //fixed neq y
    ++nfe;
    if (ipup <= 0) {
		goto L250;
    }
  ...
  //dprja_(int*, double*, double*,                   int*,    double*, double*, double*, double*, int*, myFex, myJex, ginSODACommonBlock*, double*)
    dprja_(neq,  y,       &yh[yh_offset -yh_offset], NOT_nyh, ewt,     acor,    savf,    wm,      iwm,    common,              costanti  );  //fixed neq y
    ipup = 0;
    rc = 1.;
    nslp = nst;
    crate = .7;
...
L640:
	...
	myFex(neq, &tn, y, savf, costanti); //fixed neq
    ++nfe;
    i__1 = n;
    for (i__ = 1; i__ <= i__1; ++i__) {
		/* L650: */
...
/* DECK DCFODE */
/* Subroutine */ 
__device__ int dcfode_(int PARAM_meth, double *DCFODE_elco, double *DCFODE_tesco, struct ginSODACommonBlock *common)
{
    /* System generated locals */
    int i__1 = 0;
...
L100:
    ...
    myJex(neq, &tn, y, 0, 0, &wm[3 -1], n, costanti); //fixed neq
    con = -hl0;
    i__1 = lenp;
    for (i__ = 1; i__ <= i__1; ++i__) {
		/* L120: */
...
L200:
    ...
    for (j = 1; j <= i__1; ++j) {
		...
		y[j - 1] += r__;
		fac = -hl0 / r__;
		myFex(neq, &tn, y, ftem, costanti); //fixed neq
		i__2 = n;
		for (i__ = 1; i__ <= i__2; ++i__) {
			/* L220: */
...
L400:
    ...
    myJex(neq, &tn, y, ml, mu, &wm[ml3 -1], meband, costanti); //fixed neq
    con = -hl0;
    i__1 = lenp;
    for (i__ = 1; i__ <= i__1; ++i__) {
		/* L420: */
...
/* DECK DSRCMA */
/* Subroutine */ 
//__device__ int dsrcma_(double *rsav, int *isav, int *job, struct ginSODACommonBlock *common)
//{
//    /* Initialized data */
//	
//     int lenrls = 218;
...
/* DECK XSETF */
/* Subroutine */ 
//__device__ int xsetf_(int *mflag, struct ginSODACommonBlock *common)
//{
//     int junk;
...
/* DECK XSETUN */
/* Subroutine */ 
//__device__ int xsetun_(int *lun, struct ginSODACommonBlock *common)
//{
//     int junk;
...
/* DECK IXSAV */
//__device__ int ixsav_(int ipar, int *ivalue, int iset, struct ginSODACommonBlock *common)
//{
//    /* Initialized data */
//	
//     int lunit = -1;
...
__host__ __device__ void ginSODA_Init(struct ginSODACommonBlock *common, unsigned int threads)
{
...
__global__ void ginSODA(int *neq, double *y, double *t, double *tout, int *itol, double *rtol, 
	double *atol, int *itask, int *istate, int *iopt, double *rwork, int *lrw, int *iwork, int *liw, int *jt, struct ginSODACommonBlock *common, 
	int* debug,  double* costanti, conc_t* device_X, unsigned int campione, unsigned int* s2s, 
	bool ACTIVATE_SHARED_MEMORY, bool ACTIVATE_CONSTANT_MEMORY)
{

	unsigned int tid = threadIdx.x + blockIdx.x*blockDim.x;

	if (tid>=DEV_ACTUAL_THREADS) return;
	
	double* sh_y = (double*)shared;
	double* sh_t = (double*)&sh_y[DEV_CONST_VARIABLES*blockDim.x];	

	// Load shared memory 
	if (ACTIVATE_SHARED_MEMORY) {
		for (unsigned int i=0; i<DEV_CONST_VARIABLES; i++) {		
			sh_y[threadIdx.x*DEV_CONST_VARIABLES+i] = y[tid*DEV_CONST_VARIABLES+i];		// watch out! different offsets!
		}
	...
	if (ACTIVATE_SHARED_MEMORY) {
		if (ACTIVATE_CONSTANT_MEMORY) {
			dlsoda_( neq+tid, sh_y+threadIdx.x*DEV_CONST_VARIABLES, sh_t+threadIdx.x, tout+tid, itol+tid, rtol+tid, atol+tid*DEV_CONST_VARIABLES, itask+tid, istate+tid, 
				iopt+tid, rwork+(*(lrw+tid))*tid, lrw+tid, iwork+(*(liw+tid))*tid, liw+tid,  jt+tid, common+tid, debug, costanti+tid*DEV_CONST_PARAMETERS );
		} else {
			dlsoda_( neq+tid, sh_y+threadIdx.x*DEV_CONST_VARIABLES, sh_t+threadIdx.x, tout+tid, itol+tid, rtol+tid, atol+tid*DEV_CONST_VARIABLES, itask+tid, istate+tid, 
				iopt+tid, rwork+(*(lrw+tid))*tid, lrw+tid, iwork+(*(liw+tid))*tid, liw+tid,  jt+tid, common+tid, debug, costanti+tid*DEV_CONST_PARAMETERS);
		}
	} else {
		dlsoda_( neq+tid, y+tid*DEV_CONST_VARIABLES, t+tid, tout+tid, itol+tid, rtol+tid, atol+tid*DEV_CONST_VARIABLES, itask+tid, istate+tid, 
			iopt+tid, rwork+(*(lrw+tid))*tid, lrw+tid, iwork+(*(liw+tid))*tid, liw+tid,  jt+tid, common+tid, debug, costanti+tid*DEV_CONST_PARAMETERS);
	}
	...
	if (ACTIVATE_SHARED_MEMORY) {
		for (unsigned int i=0; i<DEV_CONST_VARIABLES; i++) {		
			y[tid*DEV_CONST_VARIABLES+i] = sh_y[threadIdx.x*DEV_CONST_VARIABLES+i];		// watch out! different offsets!
		}
		t[tid]=sh_t[threadIdx.x];
	}

	// if (tid==0) printf("CS length: %d\tVariables: %d\tCampione: %d.\n", DEV_CONST_SAMPLESPECIES, DEV_CONST_VARIABLES, campione);

	// store samples
	unsigned int larg = blockDim.x * gridDim.x;		
	for (unsigned int s=0; s<DEV_CONST_SAMPLESPECIES; s++) {		
		if (ACTIVATE_SHARED_MEMORY) {
			device_X[ ACCESS_SAMPLE  ] =  sh_y[threadIdx.x*DEV_CONST_VARIABLES+s2s[s]];						
		} else {
			device_X[ ACCESS_SAMPLE  ] =  y[tid*DEV_CONST_VARIABLES+s2s[s]];			
			// if(tid==0) printf("STORING %d (index %d): %f.\n", s, ACCESS_SAMPLE, y[tid*DEV_CONST_VARIABLES+s2s[s]]);
		}
	}
...
__global__ void calculateFitnessPRR( double* samples, double* target, double* fitness, char* swarm ) {
	...
	for (unsigned int campione=0; campione<DEV_CONST_TIMESLUN; campione++) {
		...
		for (unsigned int s=num_specie_u1+num_specie_u2; s<num_specie_u1+num_specie_u2+num_specie_u3; s++) {
			// conc_u3 += samples[ tid*DEV_CONST_VARIABLES+s ];
...
__device__ inline  double fetch_target(double* source, unsigned int species, unsigned int experiment, unsigned int repetition, unsigned int sample) {
	return source[ 		
		sample*(DEV_CONST_TIMESLUN*DEV_CONST_EXPERIMENTS*DEV_CONST_REPETITIONS) + 
			experiment*(DEV_CONST_TIMESLUN*DEV_CONST_REPETITIONS) + 
				repetition*(DEV_CONST_TIMESLUN) + species ];
}
...
__device__ inline  double fetch_simulation(double* source, unsigned int species, unsigned int sample) {
	unsigned int gid = threadIdx.x + blockDim.x*blockIdx.x;	
 	return source[ blockDim.x*gridDim.x*DEV_CONST_TIMESLUN*sample + blockDim.x*gridDim.x*species+ gid ];	
}
...
//	const unsigned int WATCHED = 32;
	...
	for (unsigned int campione=1; campione<DEV_CONST_TIMESLUN; campione++) {
		for (unsigned int s=0; s<DEV_CONST_SAMPLESPECIES; s++) {
			unsigned int sid = SWARMS*DEV_CONST_TIMESLUN*campione + DEV_CONST_TIMESLUN*D + s;
...
__device__ void determine_minmax(double* const samples, unsigned int const s, 
		...
		*/
		...
		// then the fitness must be affected (we do not want damping solutions).
		// However, detecting minima and maxima is far from trivial; we use a sliding window.
		for (unsigned int i=starting_point+1; i<DEV_CONST_TIMESLUN-1; i++) {
...
__global__ void calculateFitnessNoman(double* const samples, double* fitness, double const it_ratio)  {
	...
	const unsigned int SKIP = DEV_CONST_TIMESLUN*.1;
	...
	for (unsigned int s=0; s<DEV_CONST_TIMESLUN; s++) {
		...
		double first_maxvalue = samples[larg*DEV_CONST_TIMESLUN*0 + larg*s + tid];
		double last_maxvalue  = samples[larg*DEV_CONST_TIMESLUN*0 + larg*s + tid];
		double first_minvalue = samples[larg*DEV_CONST_TIMESLUN*0 + larg*s + tid];
		double last_minvalue  = samples[larg*DEV_CONST_TIMESLUN*0 + larg*s + tid];
